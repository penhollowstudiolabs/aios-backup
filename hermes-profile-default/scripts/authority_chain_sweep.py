...
def scan_file(path):
    ...
    try:
        with open(path, encoding="utf-8", errors="replace") as f:
            head = f.read(4000)  # check banner in first 4KB
            full = f.read()
    except Exception as e:
        return [f"{path}: unreadable ({e})"]
    # status-bearing artifacts should carry a superseded marker if a live successor exists
    if "SUPERSEDED" not in head and "SUPERSEDED" not in full[:2000]:
        # only flag known-historical candidates, not necessarily every file
        pass
    ...
    for root, _, files in os.walk(inbox):
        for fn in files:
            ...
            try:
                ...
                continue
            # Only candidate status-bearing docs (has status language AND no superseded banner)
