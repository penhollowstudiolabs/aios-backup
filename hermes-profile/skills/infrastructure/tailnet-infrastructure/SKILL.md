...
5. Before locking public SSH, confirm nothing depends on public-IP SSH (cron/rsync to the public IP) and that clients (e.g. Hermes Desktop) already use tailnet names, so you don't orphan access.
   ...
   - **RECURRED 8/21 with a sharper tell:** after a Hermes backend update removed the old `serve` process, Avi's desktop "Alyosha" connection went offline. The desktop's in-app restart error read `SSH operation to root@<PUBLIC_IP> timed out` — the desktop was still dialing the **public IP** even though the `aios` / `root` / `22` form fields looked correct. **The desktop's own error message is the diagnostic that matters:** it names the *actual address being dialed*, exposing a stale saved connection record that the visible Host field does not. When a desktop connection shows offline but the Host field looks right, read the timeout/error address — if it shows the public IP, kill the stale record / re-enter the host as the tailnet name or literal tailnet IP. Do NOT advise dropping the tailnet to "fix" this: public :22 is closed by design, so the tailnet is the working route; the broken thing is a saved string. Prefer setting Host to the literal tailnet IP (e.g. `100.78.203.127`) over a hostname when a MagicDNS name keeps resolving to the stale public record, then Test SSH → Save and reconnect. Also distinguish: a desktop connection can be in **"Connect via SSH"** mode (Hermes launches on the remote over an SSH tunnel — this is what fails when the saved host is the public IP) versus a plain remote-gateway URL or a local gateway (which keeps working).
